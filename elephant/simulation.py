...
class SimulationStore:
    ...
    def _require_conn(self, conn: AsyncConnection | None) -> AsyncConnection:
        if conn is None:
    ...
    async def _fetch_run(self, simulation_id: str, conn: AsyncConnection, for_update: bool = False) -> dict:
        ...
        async with conn.cursor(row_factory=dict_row) as cur:
            await cur.execute(sql, (simulation_id,))
    ...
    async def create(self, values: List[SimulationValueInput], conn: AsyncConnection | None = None) -> str:
    ...
    async def current_value(self, simulation_id: str, conn: AsyncConnection | None = None) -> float:
        """Return the current value for a simulation and auto-advance based on call thresholds."""
        conn = self._require_conn(conn)

        try:
            run = await self._fetch_run(simulation_id, conn, for_update=True)
    ...
    async def advance(self, simulation_id: str, conn: AsyncConnection | None = None) -> float:
    ...
    async def stats(self, simulation_id: str, conn: AsyncConnection | None = None) -> dict:
    ...
    async def reset(self, conn: AsyncConnection | None = None) -> None:
        ...
        try:
            async with conn.cursor() as cur:
                await cur.execute("TRUNCATE TABLE simulation_calls;")
                await cur.execute("TRUNCATE TABLE simulation_runs;")
            await conn.commit()
        except Exception:
            await conn.rollback()
            raise
